...
pub struct UserReceiptTokenTransferContext<'info> {
    ...
    #[account(
        mut,
        seeds = [UserFundAccount::SEED, receipt_token_mint.key().as_ref(), source_token_account.owner.as_ref()],
        bump,
    )]
    /// CHECK: will be deserialized in runtime
    pub source_user_fund_account: UncheckedAccount<'info>,

    #[account(
        mut,
        seeds = [UserFundAccount::SEED, receipt_token_mint.key().as_ref(), destination_token_account.owner.as_ref()],
        bump,
    )]
    /// CHECK: will be deserialized in runtime
    pub destination_user_fund_account: UncheckedAccount<'info>,
...
impl<'info> UserReceiptTokenTransferContext<'info> {
    pub fn transfer_hook(ctx: Context<Self>, amount: u64) -> Result<()> {
        ...
        let source_token_account_owner = ctx.accounts.source_receipt_token_account.owner;
        let destination_token_account_owner = ctx.accounts.destination_receipt_token_account.owner;
        let system_program = &ctx.accounts.system_program;

        // Custom deserialize
        let mut source_user_receipt = ctx
            .accounts
            .source_user_fund_account
            .deserialize_if_exist::<UserFundAccount>("source_user_receipt")?;
        ...
        let mut destination_user_receipt = ctx
            .accounts
            .destination_user_fund_account
            .deserialize_if_exist::<UserFundAccount>("destination_user_receipt")?;
        ...
        let source_token_account_total_amount = ctx.accounts.source_receipt_token_account.amount;
        let source_user_receipt = if let Some(source_user_receipt) = &mut source_user_receipt {
            ...
            UserFundAccount::placeholder(
                source_token_account_owner,
                receipt_token_mint,
                source_token_account_total_amount,
            )
        };
        let destination_token_account_total_amount = ctx.accounts.destination_receipt_token_account.amount;
        ...
        emit!(UserTransferredReceiptToken {
            ...
            source_user: ctx.accounts.source_receipt_token_account.owner,
            source_user_receipt,
            destination_receipt_token_account: ctx.accounts.destination_receipt_token_account.key(),
            destination_user: ctx.accounts.destination_receipt_token_account.owner,
            destination_user_receipt,
        });
    ...
    fn check_token_transferring(ctx: &Context<Self>) -> Result<()> {
        ...
        if !bool::from(account_extension.transferring) {
            err!(ErrorCode::TokenNotCurrentlyTransferring)?
        }
