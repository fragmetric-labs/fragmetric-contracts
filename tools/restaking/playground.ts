...
export class RestakingPlayground extends AnchorPlayground<Restaking, KEYCHAIN_KEYS> {
    ...
    public async runOperatorProcessFundWithdrawalJob(operator: web3.Keypair = this.wallet, forced: boolean = false) {
        ...
        const {event, error} = await this.run({
            ...
            signers: [operator],
            events: ["operatorRanFund"], // , 'operatorUpdatedFundPrice'
        });
    ...
    private async runOperatorRunSingle(operator: web3.Keypair, resetCommand?: Parameters<typeof this.program.methods.operatorRun>[0], computeUnitLimit?: number, prioritizationFeeMicroLamports?: number) {
        ...
        if (fragSOLFund.operation.nextCommand) {
            for (const pubkey of fragSOLFund.operation.nextCommand.requiredAccounts) {
