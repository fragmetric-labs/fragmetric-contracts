...
export const LedgerSigner: ILedgerSignerConnector = {
    connect: async ({handler, bip32Path = `44'/501'/0'`} = {}) => {
        ...
        try {
            ...
            const publicKey = new web3.PublicKey((await solanaApp.getAddress(bip32Path)).address);
            const appConfig = await solanaApp.getAppConfiguration();

            handler?.onConnect(publicKey, appConfig.version);

            if (!appConfig.blindSigningEnabled) {
                const err = new Error(`blind signing is disabled in ledger settings`);
                handler?.onError(err);
            }

            return new LedgerSignerImpl(solanaApp, publicKey, bip32Path);
        } catch (err) {
            handler?.onError(new Error(`failed to load solana app configuration: device locked or app not open`));
            throw err;
        }
    }
}
