...
impl<'info, 'a> SignatureVerificationService<'info, 'a> {
    ...
    pub fn verify(
        ...
        let current_ix_index: usize =
            instructions::load_current_index_checked(self.instructions_sysvar)?.into();
        let previous_ix_index = current_ix_index
            .checked_sub(1)
            .ok_or(ProgramError::InvalidArgument)?;
        let ix = instructions::load_instruction_at_checked(previous_ix_index, self.instructions_sysvar)?;
