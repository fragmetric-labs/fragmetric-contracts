    ...
    fn execute<'a, 'info: 'a>(
        ...
        let (result, entry) = match &self.state {
            ...
            UnstakeLSTCommandState::GetAvailableUnstakeAccount => {
                self.execute_get_available_unstake_account(ctx, accounts, &self.items)?
            }
            UnstakeLSTCommandState::Unstake => self.execute_unstake(ctx, accounts, &self.items)?,
            UnstakeLSTCommandState::RequestUnstake(withdraw_stake_items) => {
                self.execute_request_unstake(ctx, accounts, &self.items, withdraw_stake_items)?
            }
        };
...
impl UnstakeLSTCommand {
    ...
    fn execute_prepare<'info>(
        ...
        let entry = match pricing_source {
            Some(TokenPricingSource::SPLStakePool { .. }) => {
                ...
                Self {
                    items,
                    state: UnstakeLSTCommandState::GetAvailableUnstakeAccount,
                }
            ...
            Some(TokenPricingSource::MarinadeStakePool { .. }) => {
                ...
                Self {
                    state: UnstakeLSTCommandState::RequestUnstake(vec![]),
                    items,
                }
                .with_required_accounts(required_accounts)
            }
            Some(TokenPricingSource::SanctumSingleValidatorSPLStakePool { .. }) => {
                let required_accounts = SanctumSingleValidatorSPLStakePoolService::find_accounts_to_get_available_unstake_account(pool_account)?;
    ...
    fn execute_get_available_unstake_account<'info>(
        ...
        let entry = match token_pricing_source {
            Some(TokenPricingSource::SPLStakePool { address }) => self
                .spl_stake_pool_get_available_withdrawals::<SPLStakePool>(
                    ctx, accounts, items, item, address,
                ),
            Some(TokenPricingSource::SanctumSingleValidatorSPLStakePool { address }) => {
                self.spl_stake_pool_get_available_withdrawals::<SanctumSingleValidatorSPLStakePool>(
                    ctx, accounts, items, item, address,
                )
            }
            _ => err!(ErrorCode::FundOperationCommandExecutionFailedException),
    ...
    fn spl_stake_pool_get_available_withdrawals<'info, T: SPLStakePoolInterface>(
        &self,
        ctx: &mut OperationCommandContext<'info, '_>,
        accounts: &[&'info AccountInfo<'info>],
        items: &[UnstakeLSTCommandItem],
        ...
        let &[pool_program, pool_account, pool_token_mint, pool_token_program, reserve_stake_account, validator_list_account, ..] =
            accounts
        else {
        ...
        let available_withdrawals = spl_stake_pool_service.get_available_withdrawals(
            reserve_stake_account,
            validator_list_account,
            current_item.allocated_token_amount,
        )?;
        ...
        let entry = match available_withdrawals {
            AvailableWithdrawals::Reserve => {
                let accounts_to_withdraw_sol =
                    SPLStakePoolService::<T>::find_accounts_to_withdraw_sol(pool_account)?;
                ...
                .into_iter()
                .chain(accounts_to_withdraw_sol);

                Self {
                    items: items.to_vec(),
                    state: UnstakeLSTCommandState::Unstake,
                }
                .with_required_accounts(required_accounts)
            }
            AvailableWithdrawals::Validators(validator_stake_items) => {
                let withdraw_stake_items = validator_stake_items
                    .iter()
                    .map(
                        |&(validator_stake_account, token_amount)| WithdrawStakeItem {
                ...
                let accounts_to_withdraw_stake =
                    SPLStakePoolService::<T>::find_accounts_to_withdraw_stake(pool_account)?;
                ...
                let validator_stake_accounts = validator_stake_items
                    .iter()
                    .map(|&(validator_stake_account, _)| (validator_stake_account, true));

                let required_accounts = [(fund_supported_token_reserve_account, true)]
                    .into_iter()
                    .chain(accounts_to_withdraw_stake)
                    .chain(fund_stake_accounts)
                    .chain(validator_stake_accounts);

                Self {
                    items: items.to_vec(),
                    state: UnstakeLSTCommandState::RequestUnstake(withdraw_stake_items),
                }
                .with_required_accounts(required_accounts)
            }
        };
