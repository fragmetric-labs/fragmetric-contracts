...
impl<'info> NormalizedTokenPoolAdapter<'info> {
    ...
    fn get_denominated_amount_per_supported_token(&self) -> Result<u64> {
        10u64
            .checked_pow(self.supported_token_mint.decimals as u32)
            .ok_or_else(|| error!(ErrorCode::CalculationArithmeticException))
    }
    ...
    pub(super) fn withdraw(
        ...
        let supported_token_amount = crate::utils::get_proportional_amount(
            ...
            self.get_denominated_amount_per_supported_token()?,
        ...
        token_interface::burn(
            CpiContext::new_with_signer(
                ...
                token_interface::Burn {
                    mint: self.normalized_token_mint.to_account_info(),
                    from: normalized_token_account.to_account_info(),
        ...
        token_interface::transfer_checked(
            CpiContext::new_with_signer(
                self.supported_token_program.to_account_info(),
                token_interface::TransferChecked {
                    from: self.supported_token_lock_account.to_account_info(),
                    mint: self.supported_token_mint.to_account_info(),
    ...
    pub(in crate::modules) fn into_pool_account(self) -> Box<Account<'info, NormalizedTokenPoolAccount>> {
        self.normalized_token_pool_account
    }
}
