...
use crate::modules::restaking::jito::{JitoRestakingVaultContext, JitoRestakingVault, JitoVaultOperatorDelegation};
...
impl JitoRestakingVault {
    ...
    pub fn get_vault_operator_delegation_key(ctx: JitoRestakingVaultContext, operator: &Pubkey) -> Pubkey {
        let (vault_operator_delegation_key, _, _) = VaultOperatorDelegation::find_program_address(ctx.vault_program.key, ctx.vault.key, operator);
        vault_operator_delegation_key
    }

    pub fn get_status(vault: AccountInfo, vault_operators_delegation: &[AccountInfo]) -> Result<JitoRestakingVault> {
        ...
        let vault_receipt_token_enqueued_for_withdrawal_amount = vault.vrt_enqueued_for_cooldown_amount();
        let vault_receipt_token_claimable_amount = vault.vrt_ready_to_claim_amount();
        let vault_receipt_token_unrestake_pending_amount = vault_receipt_token_withdrawal_pending_amount.checked_add(vault_receipt_token_enqueued_for_withdrawal_amount).unwrap();
        ...
        let supported_token_enqueued_for_undelegate_amount = vault_delegation_state.enqueued_for_cooldown_amount();
        let supported_token_undelegate_pending_amount = supported_token_cooling_down_amount.checked_add(supported_token_enqueued_for_undelegate_amount).unwrap();
        ...
        for vault_operator_delegation in vault_operators_delegation {
            ...
            let vault_operator_delegation = VaultOperatorDelegation::try_from_slice_unchecked(vault_operator_delegation_data)?;

            let vault_operator_delegation_state = vault_operator_delegation.delegation_state;
            let operator_supported_token_delegated_amount = vault_operator_delegation_state.staked_amount();
            let operator_supported_token_cooling_down_amount = vault_operator_delegation_state.cooling_down_amount();
            let operator_supported_token_enqueued_for_undelegate_amount = vault_operator_delegation_state.enqueued_for_cooldown_amount();
            let operator_supported_token_undelegate_pending_amount = operator_supported_token_cooling_down_amount.checked_add(operator_supported_token_enqueued_for_undelegate_amount).unwrap();
