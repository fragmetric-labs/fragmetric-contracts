...
use crate::modules::{fund, staking, normalize, pricing};
use crate::errors;
use crate::events;
use crate::constants::{ADMIN_PUBKEY};
use crate::modules::normalize::NormalizedTokenPoolAdapter;
...
pub fn process_run<'info>(
    ...
    {
        let mut fund_supported_token_account_to_stake_parsed = InterfaceAccount::<TokenAccount>::try_from(fund_supported_token_account_to_stake)?;
        ...
        if staking_lamports > 0 {
            let before_fund_supported_token_amount = fund_supported_token_account_to_stake_parsed.amount;
            ...
            let minted_supported_token_amount = fund_supported_token_account_to_stake_parsed.amount - before_fund_supported_token_amount;
            let fund_supported_token_info = fund_account.get_supported_token_mut(fund_supported_token_account_to_stake_parsed.mint)?;
            fund_supported_token_info.set_operation_reserved_amount(fund_supported_token_info.get_operation_reserved_amount().checked_add(minted_supported_token_amount).unwrap());
            msg!("staked {} sol to mint {} tokens", staking_lamports, minted_supported_token_amount);

            require_gte!(minted_supported_token_amount, staking_lamports.div_ceil(2));
            require_eq!(fund_supported_token_info.get_operation_reserved_amount(), fund_supported_token_account_to_stake_parsed.amount);
        }
    }

    // create pricing calculator
    let mut pricing_source_map = fund::create_pricing_source_map(fund_account, pricing_source_accounts)?;
    pricing_source_map.insert(normalized_token_mint.key(), (TokenPricingSource::NormalizedTokenPool {
    ...
    {
        let normalized_token_pool_account_parsed = Account::<normalize::NormalizedTokenPoolAccount>::try_from(normalized_token_pool_account)?;
        let mut fund_supported_token_account_to_normalize_parsed = InterfaceAccount::<TokenAccount>::try_from(fund_supported_token_account_to_normalize)?;
        let fund_supported_token_account_authority_to_normalize_parsed = Account::<fund::SupportedTokenAuthority>::try_from(fund_supported_token_account_authority_to_normalize)?;
        let fund_supported_token_info_to_normalize = fund_account.get_supported_token_mut(fund_supported_token_account_to_normalize_parsed.mint)?;
        let mut fund_normalized_token_account_parsed = InterfaceAccount::<TokenAccount>::try_from(fund_normalized_token_account)?;

        let normalizing_supported_token_amount = fund_supported_token_info_to_normalize.get_operation_reserved_amount();
        if normalizing_supported_token_amount > 0 {
            let before_fund_supported_token_amount = fund_supported_token_account_to_normalize_parsed.amount;
            let before_fund_normalized_token_amount = fund_normalized_token_account_parsed.amount;
            let normalized_token_mint_parsed = InterfaceAccount::<Mint>::try_from(normalized_token_mint)?;
            let normalized_token_program_parsed = Program::<Token>::try_from(normalized_token_program)?;
            let fund_supported_token_mint_to_normalize_parsed = InterfaceAccount::<Mint>::try_from(fund_supported_token_mint_to_normalize)?;
            let fund_supported_token_program_to_normalize_parsed = Interface::<TokenInterface>::try_from(fund_supported_token_program_to_normalize)?;
            let mut normalized_token_pool_supported_token_lock_account_parsed = InterfaceAccount::<TokenAccount>::try_from(normalized_token_pool_supported_token_lock_account)?;
            let mut normalizer = NormalizedTokenPoolAdapter::new(
            ...
            normalize::normalize_supported_token(
                ...
                &[fund_supported_token_account_authority_to_normalize_parsed.get_signer_seeds().as_ref()],
            ...
            fund_supported_token_info_to_normalize.set_operation_reserved_amount(fund_supported_token_info_to_normalize.get_operation_reserved_amount() - normalizing_supported_token_amount);
            let normalized_fund_supported_token_amount = before_fund_supported_token_amount - fund_supported_token_account_to_normalize_parsed.amount;
            require_eq!(normalized_fund_supported_token_amount, normalizing_supported_token_amount);

            fund_normalized_token_account_parsed.reload()?;
            let minted_normalized_token_amount = fund_normalized_token_account_parsed.amount - before_fund_normalized_token_amount;
            fund_supported_token_info_to_normalize.set_operating_amount(fund_supported_token_info_to_normalize.get_operating_amount() + normalizing_supported_token_amount);
            msg!("normalized {} tokens to mint {} normalized tokens", normalizing_supported_token_amount, minted_normalized_token_amount);
