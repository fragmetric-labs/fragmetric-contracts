...
impl<'info> NormalizedTokenPoolAdapter<'info> {
    pub(super) fn new(
        ...
        Ok(Self {
            ...
            normalized_token_account: Box::new(InterfaceAccount::try_from(&accounts[2])?),
            supported_token_mint: Box::new(InterfaceAccount::try_from(&accounts[3])?),
            supported_token_program: Box::new(Interface::try_from(&accounts[4])?),
            supported_token_lock_account: Box::new(InterfaceAccount::try_from(&accounts[5])?),
        })
    }
