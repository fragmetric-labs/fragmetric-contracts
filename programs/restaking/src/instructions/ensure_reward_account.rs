...
impl<'info> RewardEnsure<'info> {
    pub fn realloc_if_needed(ctx: Context<Self>, desired_size: Option<u32>, asserted: bool) -> Result<()> {
        ...
        if remaining_increase > 0 {
            ...
            let increase = if remaining_increase > max_increase {
                if asserted {
                    return Err(ErrorCode::RewardInvalidAccountSize)?
                }
                max_increase
            } else {
                remaining_increase
